    ...
    try {
      ...
      if (data.token) {
        ...
        showAlert({
          ...
          message: data.error || `Failed to register with ${provider}`,
          tone: 'danger',
        });
      }
    } catch (error) {
      showAlert({
        title: 'Error',
        message: `An error occurred during ${provider} registration.`,
        tone: 'danger',
      });
    } finally {
      setIsLoading(false);
      ...
      >
        ...
        >
          ...
          <View className="flex-row items-center mb-6">
            ...
            <Text className="mx-4 text-slate-400 font-bold text-[12px] uppercase tracking-widest">Or sign up with</Text>
            <View className="flex-1 h-[1px] bg-slate-200" />
          </View>
